...
class Auth extends CI_Controller {
    ...
    public function update_attendance($student_id){
        $date = date("d-m-Y");
        $student = $this->db->get_where('students', array('id'=>$student_id))->row();
        $school = $this->db->get_where('schools', array('id'=>$student->school_id))->row();
        ...
        $this->db->where('S.id', $student->id);
        $this->db->where('E.academic_year_id', $school->academic_year_id);

        $student_running_session =  $this->db->get()->row();
        //Getting ready data for attendance
        if($student_running_session){
            $status = 'P';
            $condition['school_id'] = $student_running_session->school_id;
            $condition['student_id'] = $student_id;
            $condition['class_id'] = $student_running_session->class_id;

            if($student_running_session->section_id){
                $condition['section_id'] = $student_running_session->section_id;
            }

            $condition['month'] = date('m', strtotime($date));
            $condition['year'] = date('Y', strtotime($date));
            ...
            $field = 'day_' . abs(date('d', strtotime($date)));
            if ($this->db->update('student_attendances', array($field => $status, 'modified_at'=>date('Y-m-d H:i:s')), $condition)) {
                echo TRUE;
            } else {
                echo FALSE;
            }
